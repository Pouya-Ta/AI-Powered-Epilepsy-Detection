...
New_dir = r"C:\Users\Pouya\Documents\university\Bachelor - AUT\Final Project\Data Set\Nowrous\DT"
...
for filename in os.listdir(data_dir):
    if filename.startswith("final_train_") and filename.endswith(".csv"):
        ...
        plt.ylabel("True")
        plt.tight_layout()
        plt.savefig(os.path.join(cm_plot_dir, f"confusion_matrix_{patient_id}.png"))
        plt.close()
        ...
        plt.ylim(0, 1)
        plt.ylabel("Accuracy")
        plt.title(f"Accuracy Comparison - {patient_id}")
        plt.tight_layout()
        plt.savefig(os.path.join(acc_plot_dir, f"accuracy_{patient_id}.png"))
        plt.close()
        ...
        plt.title(f"Decision Tree - {patient_id}")
        plt.savefig(os.path.join(tree_plot_dir, f"tree_{patient_id}.png"))
        plt.close()

        print(f"✅ Finished {patient_id}: Test Acc = {test_acc:.2f}, F1 = {f1:.2f}")
